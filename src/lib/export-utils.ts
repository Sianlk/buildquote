...
export function exportToPDF(
  ...
  // Scale indicator for drawings
  if (scale) {
    doc.text(`Scale: ${scale}`, pageWidth - margin, 16, { align: "right" });
  }
  
  // Reference/document number
  const docRef = `REF: BQ-${Date.now().toString(36).toUpperCase()}`;
  doc.text(docRef, pageWidth - margin, 22, { align: "right" });
  ...
  columns.forEach((col, i) => {
    ...
    doc.text(truncateText(col.label, colWidth - 4), x, y + 5.5);
  });
  y += rowHeight;

  // Table rows with gridlines
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text("Generated by BuildQuote UK", margin, pageHeight - 12);
    doc.text(new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }), pageWidth - margin, pageHeight - 12, { align: "right" });
    
    // Disclaimer on first page
    if (i === 1) {
      doc.setFontSize(6);
      doc.text("For estimating purposes only. Not a substitute for professional certification.", pageWidth / 2, pageHeight - 7, { align: "center" });
    }
  }
