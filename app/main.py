"""
BuildQuote — FastAPI Backend
Domain: construction
AI Focus: cost estimation, material calculation, project scheduling
Features: AI Quote Builder, Material AI, Project Timeline, Client Portal, Cost Analytics
"""
...
async def lifespan(app: FastAPI):
    print(f"[BuildQuote] Starting up...")
    yield
    print(f"[BuildQuote] Shutting down...")

# ── App ───────────────────────────────────────────────────────────────────────
app = FastAPI(
    title="BuildQuote API",
    description="Smart construction quoting & management",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
...
async def root():
    return {
        "service": "BuildQuote",
        "tagline": "Smart construction quoting & management",
        "domain": "construction",
        "version": "1.0.0",
        "docs": "/docs",
        "status": "live",
...
Focus area: cost estimation, material calculation, project scheduling.
Tagline: Smart construction quoting & management.
Provide accurate, helpful, domain-specific responses."""
    ...
    if not openai_key:
        ...
        content = f"[DEV MODE] BuildQuote AI received: {request.prompt[:100]}"
...
async def run_agent(task: AgentTaskRequest, req: Request):
    ...
    AI_REQUESTS.labels("gpt-4o", "construction").inc()
    return {
...
async def websocket_ai(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            await websocket.send_text(f"[BuildQuote AI] Processing: {data[:50]}")
            await asyncio.sleep(0.1)
            await websocket.send_text(f"[STREAM] Domain: construction | Response ready")
